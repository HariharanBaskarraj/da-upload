...
class FileDeliveryService:
        ...
        try:
            ...
            if existing:
                ...
                self.file_tracker_table.update_item(
                    Key={'DA_ID': da_id, 'AssetId': asset_id},
                    UpdateExpression=update_expr,
                    ExpressionAttributeNames=expr_attr_names,
                    ExpressionAttributeValues=expr_attr_values
                )
                
                logger.info(f"Updated file tracker: DA={da_id}, Asset={filename}, Status={file_status}")
            else:
                item = {
                    'DA_ID': da_id,
                    'AssetId': asset_id,
                ...
                logger.info(f"Created file tracker: DA={da_id}, Asset={filename}, Status=NEW")
            ...
            logger.error(f"Error tracking file delivery: {e}")
            raise
    ...
    def update_component_delivery_status(self, da_id: str, component_id: str, title_id: str, version_id: str) -> None:
        try:
            ...
            if 'Item' not in response:
                logger.warning(f"Component not found: DA={da_id}, Component={component_id}")
                return
            ...
            if not expected_assets:
                logger.info(f"No expected assets found for component {component_id}")
                return
            
            delivered_asset_ids = {f['AssetId'] for f in component_files}
            expected_asset_ids = {a['AssetId'] for a in expected_assets}
            ...
            logger.info(f"Updated component delivery status: DA={da_id}, Component={component_id}, Status={delivery_status}")
    ...
    def update_da_delivery_status(self, da_id: str) -> None:
        try:
            ...
            if not components:
                logger.warning(f"No components found for DA: {da_id}")
                return
            ...
            if 'Item' not in da_response:
                logger.warning(f"DA not found: {da_id}")
                return
            ...
            logger.info(f"Updated DA delivery status: DA={da_id}, Status={delivery_status}")
            
        except ClientError as e:
            logger.error(f"Error updating DA status: {e}")
            raise

    def _get_file_tracker(self, da_id: str, asset_id: str) -> Optional[Dict]:
        try:
            response = self.file_tracker_table.get_item(
                Key={'DA_ID': da_id, 'AssetId': asset_id}
            )
