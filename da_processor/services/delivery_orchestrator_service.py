...
from da_processor.utils.date_utils import get_current_zulu, parse_date
...
class DeliveryOrchestratorService:
        ...
        try:
            ...
            for asset_data in assets:
                asset_dict = {
                    ...
                    'Folder_Path': asset_data.get('file_path', '').rsplit('/', 1)[0] + '/' if '/' in asset_data.get('file_path', '') else '',
            ...
            new_or_revised_count = sum(
                ...
                if asset.get('file_status') in ['NEW', 'REVISED']
            )
    ...
    def _enrich_manifest_with_file_status(self, manifest: Dict, da_id: str) -> Dict:
        ...
        file_status_map = {f['AssetId']: f.get('File_Status', 'NEW') for f in tracked_files}
        ...
        for asset in manifest.get('assets', []):
            ...
            if file_status == 'NO_CHANGE':
                asset_copy['file_status'] = 'No Change'
            elif file_status == 'REVISED':
